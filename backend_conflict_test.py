...
def test_conflict_resolution_flow():
    ...
    # Find CONFLICT-TEST-LOC in the updated report
    ...
    if not conflict_location_2:
        print("❌ CONFLICT-TEST-LOC not found in updated bin-wise report")
        return False
    ...
    # Find CONFLICT-TEST-LOC in the final report
    final_location = None
    for location in binwise_data_3.get('locations', []):
        if location.get('location') == 'CONFLICT-TEST-LOC':
            final_location = location
            break
    
    if not final_location:
        print("❌ CONFLICT-TEST-LOC not found in final bin-wise report")
        return False
