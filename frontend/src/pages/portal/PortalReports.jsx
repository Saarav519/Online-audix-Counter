...
export default function PortalReports() {
  ...
  const fetchReport = useCallback(async (forceRefresh = false) => {
    ...
    // On forced refresh, invalidate the cached entry so next background hit is fresh too
    if (forceRefresh) {
  ...
  const saveRecoAdjustment = async (params) => {
    try {
      // Reco is allowed in:
      //  • Warehouse:      only consolidated view
      //  • Store:          consolidated view OR any single-session view (Option B)
      //  • Cycle Count:    only Day-wise Detailed view (per spec)
      const allowed = isCycleCountClient
        ? (isCcDayView && reportType === 'detailed')
        : (selectedSession === '__consolidated__' || isStoreClient);
      if (!allowed || !selectedClient) {
        toast.error(isCycleCountClient
          ? 'Reco can only be edited from Day-wise Detailed Report'
          : 'Reco is only available in consolidated view');
        return;
      }
      const body = { client_id: selectedClient, ...params };
      // Movement / Audit Log attribution + cycle context
      try {
        const _u = JSON.parse(localStorage.getItem('auditPortalUser') || localStorage.getItem('portalUser') || '{}');
        if (_u && _u.id) body.user_id = _u.id;
        if (_u && _u.username) body.username = _u.username;
      } catch { /* ignore */ }
      if (selectedSession) body.session_id = selectedSession;
      if (isCycleCountClient) {
        body.module = 'cycle_count';
        const _selDay = (selectedDay && selectedDay !== 'consolidated') ? selectedDay : '';
        if (_selDay) {
          const _proj = selectedProjectObj;
          const _day = (_proj?.days || []).find(d => d.id === _selDay);
          if (_day?.day_no != null) body.cycle_day = _day.day_no;
        }
      } else {
      ...
      if (response.ok) {
        ...
        // every report type in the warehouse/store flow.
        if (isCycleCountClient) {
          reportCache.current = {};
        } else {
          Object.keys(reportCache.current).forEach(key => {
  ...
  // requested for Store Wise. Warehouse stays Reco-only-in-consolidated.
  const isStoreClient = selectedClientObj?.client_type === 'store';
  ...
  // Numeric Day No (for the Movement / Audit Log). Pulled from the
  // selected cycle-count day so per-day edits/recos are attributable.
  const selectedDayNo = useMemo(() => {
    if (!isCycleCountClient || !selectedDay || selectedDay === 'consolidated') return null;
    const _proj = selectedProjectObj;
    const _day = (_proj?.days || []).find(d => d.id === selectedDay);
    return _day?.day_no != null ? _day.day_no : null;
  }, [isCycleCountClient, selectedDay, selectedProjectObj]);
  ...
  // Cycle-count Day-wise view (a specific day selected) — Reco is editable
  // only here AND only in the Detailed report. Every other cycle-count
  // surface shows Reco/Final Qty as read-only but still visible.
  const isCcDayView = isCycleCountClient && !!selectedDay && selectedDay !== 'consolidated';
  // Unified flag — used to decide whether Reco/Final Qty columns + reco
  // editing UI should be shown. For Cycle Count we always show these
  // columns (day or consolidated) so subtotals & values stay consistent
  // across every view; for Store we also always show them (Option B —
  // Reco available in single-session AND consolidated). Warehouse keeps
  // the original "consolidated only" gate.
  const showRecoFinalCols = isConsolidatedView || isCycleCountClient || isStoreClient;
  ...
  const isRecoEditable = useMemo(() => {
    ...
    // Store-type single-session: editing allowed when current report
    // matches the session's variance mode (mirrors the consolidated rule).
    // Consolidated view (selectedSession === '__consolidated__') keeps
    // legacy behaviour for both Warehouse + Store.
    const inEligibleSessionView = isConsolidatedView || isStoreClient;
    if (!inEligibleSessionView) return false;
    ...
    return false;
  }, [isConsolidatedView, isStoreClient, sessionInfo, reportType]);
    ...
    switch (reportType) {
      case 'bin-wise':
        return [
          ...
          ...(showRecoFinalCols ? [{ key: 'final_qty', label: 'Final Qty' }] : []),
  ...
  }, [reportType, showRecoFinalCols, extraColumns, schemaValueFields, isCycleCountClient]);
  ...
  return (
    <div className="p-3 md:p-4 lg:p-5">
      ...
      <FullScreenReport
        ...
        isConsolidated={showRecoFinalCols}
        isRecoEditable={isCycleCountClient ? (isCcDayView && reportType === 'detailed') : ((isConsolidatedView || isStoreClient) && (
          (reportType === 'detailed' && sessionInfo?.variance_mode === 'bin-wise') ||
          (reportType === 'barcode-wise' && sessionInfo?.variance_mode === 'barcode-wise') ||
          (reportType === 'article-wise' && sessionInfo?.variance_mode === 'article-wise')
        ))}
          ...
          <tbody>
            ...
            {rowVirtualizer.getVirtualItems().map((virtualRow) => {
              ...
              <tr key={i} className="border-b border-gray-100 hover:bg-gray-50">
                ...
                <td className="py-2 px-3 font-mono"><BarcodeEditCell value={row.barcode} row={row} clientId={clientId} reportType="detailed" field="barcode" onEditComplete={onRefresh} readOnly={barcodeReadOnly} sessionId={auditSessionId} cycleDay={auditCycleDay} module={auditModule} /></td>
        ...
        <table className="min-w-full text-sm report-table">
          ...
          <tbody>
            ...
            {rowVirtualizer.getVirtualItems().map((virtualRow) => {
              ...
              return (
              <tr key={i} className="border-b border-gray-100 hover:bg-gray-50">
                <td className="py-2 px-3 font-mono text-xs"><BarcodeEditCell value={row.barcode} row={row} clientId={clientId} reportType="barcode-wise" field="barcode" onEditComplete={onRefresh} compact readOnly={barcodeReadOnly} sessionId={auditSessionId} cycleDay={auditCycleDay} module={auditModule} /></td>
                <td className="py-2 px-3">{row.description || '-'}</td>
            ...
            {data.report.map((row, i) => (
              <React.Fragment key={i}>
                <tr className={`border-b border-gray-100 hover:bg-gray-50 cursor-pointer ${row.article_code === 'UNMAPPED' ? 'bg-red-50/50' : ''}`} onClick={() => toggleRow(i)}>
                  ...
                  <td className="py-2 px-3 font-mono text-xs font-medium"><BarcodeEditCell value={row.article_code} row={row} clientId={clientId} reportType="article-wise" field="article_code" onEditComplete={onRefresh} compact readOnly={barcodeReadOnly} sessionId={auditSessionId} cycleDay={auditCycleDay} module={auditModule} /></td>
