  ...
  const exportCSV = () => {
    ...
    if (reportType === 'bin-wise') {
      ...
      if (isConsolidatedView) {
        ...
        csv = 'Location,Barcode,Description,Category,Stock Qty,Physical Qty,Diff Qty,Accuracy %,Remarks\n';
        rows.forEach(row => {
          csv += `"${row.location}","${row.barcode}","${row.description}","${row.category}",${row.stock_qty},${row.physical_qty},${row.diff_qty},${row.accuracy_pct}%,"${row.remark}"\n`;
        });
      }
      ...
      if (isConsolidatedView) {
        ...
        csv = 'Barcode,Description,Category,Stock Qty,Stock Value,Physical Qty,Diff Qty,Diff Value,Accuracy %,Remarks\n';
        rows.forEach(row => {
          csv += `"${row.barcode}","${row.description}","${row.category}",${row.stock_qty},${row.stock_value || 0},${row.physical_qty},${row.diff_qty},${row.diff_value || 0},${row.accuracy_pct}%,"${row.remark}"\n`;
        });
      }
      ...
      if (isConsolidatedView) {
        ...
        csv = 'Article Code,Article Name,Category,Barcodes,Stock Qty,Physical Qty,Diff Qty,Diff Value,Accuracy %,Remarks\n';
        rows.forEach(row => {
          csv += `"${row.article_code}","${row.article_name}","${row.category}","${(row.barcodes || []).join('; ')}",${row.stock_qty},${row.physical_qty},${row.diff_qty},${row.diff_value || 0},${row.accuracy_pct}%,"${row.remark}"\n`;
        });
      }
...
function DetailedTable({ data, getVarianceIcon, getVarianceClass, getAccuracyClass, getRemarkIcon, sortConfig, onSort, columnFilters, onFilterChange, numericFilters, onNumericFilterChange, getColumnValues, onSaveReco, isConsolidated }) {
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
      <div className="p-4 border-b border-gray-200">
        ...
        <table className="w-full text-xs">
          <thead className="bg-gray-50">
            <tr>
              ...
              <SortableHeader column="physical_qty" label="Physical" align="right" sortConfig={sortConfig} onSort={onSort} allValues={getColumnValues('physical_qty')} activeFilters={columnFilters} onFilterChange={onFilterChange} numericFilters={numericFilters} onNumericFilterChange={onNumericFilterChange} />
              {isConsolidated && <th className="py-3 px-3 text-right text-xs font-semibold text-blue-700 bg-blue-50/50">Reco</th>}
              {isConsolidated && <SortableHeader column="final_qty" label="Final Qty" align="right" sortConfig={sortConfig} onSort={onSort} allValues={getColumnValues('final_qty')} activeFilters={columnFilters} onFilterChange={onFilterChange} numericFilters={numericFilters} onNumericFilterChange={onNumericFilterChange} />}
...
// ============ Barcode-wise Table ============
function BarcodeWiseTable({ data, getVarianceIcon, getVarianceClass, getAccuracyClass, getRemarkIcon, sortConfig, onSort, columnFilters, onFilterChange, numericFilters, onNumericFilterChange, getColumnValues, onSaveReco, isRecoEditable, isConsolidated }) {
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
      <div className="p-4 border-b border-gray-200">
