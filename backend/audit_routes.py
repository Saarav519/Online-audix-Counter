...
# Create routers for audit module (will be included in main server.py)
# Note: prefixes will be added when including in main app
audit_api_router = APIRouter()
audit_portal_router = APIRouter()
audit_sync_router = APIRouter()

# Keep old names for compatibility within this file
api_router = audit_api_router
portal_router = audit_portal_router
sync_router = audit_sync_router
...
class TTLCache:
    """Simple in-memory cache with TTL. Thread-safe for async."""
    ...
    def get(self, key):
        ...
        if entry and (_time.time() - entry[1]) < self._ttl:
            return entry[0]
        if entry:
            del self._store[key]
        return None
...
# Cache instances
_master_cache = TTLCache(ttl_seconds=600)    # 10 min - master products rarely change
_expected_cache = TTLCache(ttl_seconds=600)  # 10 min - expected stock doesn't change
...
async def get_cached_master(client_id):
    ...
    if cached is not None:
        return cached
    records = await db.master_products.find({"client_id": client_id}, {"_id": 0}).to_list(500000)
    _master_cache.set(key, records)
    return records

async def get_cached_expected(session_id):
    """Load expected stock with cache."""
    key = f"expected_{session_id}"
    cached = _expected_cache.get(key)
    if cached is not None:
        return cached
    records = await db.expected_stock.find({"session_id": session_id}, {"_id": 0}).to_list(100000)
    _expected_cache.set(key, records)
    return records

async def get_cached_reco(client_id):
    """Load reco adjustments with cache."""
    key = f"reco_{client_id}"
    cached = _reco_cache.get(key)
    if cached is not None:
        return cached
    records = await db.reco_adjustments.find({"client_id": client_id}, {"_id": 0}).to_list(100000)
    _reco_cache.set(key, records)
    return records

async def get_master_by_barcode(client_id):
    """Get master products indexed by barcode (cached)."""
    key = f"master_by_barcode_{client_id}"
    cached = _master_cache.get(key)
    if cached is not None:
        return cached
    master = await get_cached_master(client_id)
    by_barcode = {}
    for p in master:
        bc = p.get("barcode", "")
        if bc:
            by_barcode[bc] = p
    _master_cache.set(key, by_barcode)
    return by_barcode
...
# Location Master Model (imported at client level - valid locations)
...
# Fields that are always rendered as fixed columns in reports (not in extra_columns)
STANDARD_IMPORT_FIELD_NAMES = {"barcode", "description", "category", "mrp", "cost", "article_code", "article_name"}
# Fields that are always rendered as fixed columns in reports (not in extra_columns)
REPORT_FIXED_COLUMNS = {"barcode", "description", "category", "mrp", "cost"}
...
def parse_number(value):
    """Parse number from string, handles comma-formatted values like '2,000.00'"""
    if not value:
        return 0.0
    try:
...
async def download_schema_template(client_id: str, template_type: str = "master", variance_mode: str = "bin-wise"):
    ...
    if template_type == "stock":
        # Location column only for bin-wise variance mode
        if variance_mode == "bin-wise" and "location" not in enabled_fields:
...
async def import_master_products(client_id: str, file: UploadFile = File(...)):
    ...
    for row in reader:
        ...
        master = MasterProduct(
            ...
            mrp=parse_number(norm_row.get('mrp', 0)),
            cost=parse_number(norm_row.get('cost', 0)),
        )
...
@portal_router.post("/clients/{client_id}/import-location-master")
async def import_location_master(client_id: str, file: UploadFile = File(...)):
    """Import location master CSV at client level. Replaces existing locations on re-upload.
    Expected columns: Location Code, Location Name, Zone, Floor, Area, Type"""
    client = await db.clients.find_one({"id": client_id})
    if not client:
        raise HTTPException(status_code=404, detail="Client not found")
    ...
    await db.location_master.delete_many({"client_id": client_id})

    records = []
    for row in reader:
        norm_row = {k.strip().lower().replace(' ', '_'): v.strip() for k, v in row.items()}

        loc_code = norm_row.get('location_code', norm_row.get('location', norm_row.get('loc_code', norm_row.get('code', '')))).strip()
        if not loc_code:
            continue
        ...
        loc = LocationMaster(
            ...
            location_name=norm_row.get('location_name', norm_row.get('name', loc_code)),
            zone=norm_row.get('zone', ''),
            floor=norm_row.get('floor', ''),
            area=norm_row.get('area', ''),
            location_type=norm_row.get('location_type', norm_row.get('type', '')),
        )
        doc = loc.model_dump()
        doc['imported_at'] = doc['imported_at'].isoformat()
        records.append(doc)
    ...
    if records:
        await db.location_master.insert_many(records)

    await db.clients.update_one(
        {"id": client_id},
        {"$set": {"location_master_imported": True, "location_count": len(records)}}
    )

    return {
        "message": f"Imported {len(records)} locations",
        "location_count": len(records)
    }


@portal_router.get("/clients/{client_id}/location-master")
async def get_location_master(client_id: str, limit: int = 1000, skip: int = 0):
    """Get location master for a client with pagination"""
    total = await db.location_master.count_documents({"client_id": client_id})
    records = await db.location_master.find(
        {"client_id": client_id}, {"_id": 0}
    ).skip(skip).limit(limit).to_list(limit)
    return {"locations": records, "total": total, "limit": limit, "skip": skip}


@portal_router.get("/clients/{client_id}/location-master/stats")
async def get_location_master_stats(client_id: str):
    """Get location master statistics"""
    total = await db.location_master.count_documents({"client_id": client_id})
    zones = await db.location_master.distinct("zone", {"client_id": client_id})
    zones = [z for z in zones if z]
    floors = await db.location_master.distinct("floor", {"client_id": client_id})
    floors = [f for f in floors if f]
    types = await db.location_master.distinct("location_type", {"client_id": client_id})
...
@portal_router.delete("/clients/{client_id}/location-master")
async def clear_location_master(client_id: str):
    """Clear all location master entries for a client"""
    result = await db.location_master.delete_many({"client_id": client_id})
    await db.clients.update_one(
        {"id": client_id},
        {"$set": {"location_master_imported": False, "location_count": 0}}
    )
    return {"message": f"Cleared {result.deleted_count} locations", "deleted_count": result.deleted_count}
...
async def import_client_stock(client_id: str, file: UploadFile = File(...)):
    ...
    if schema:
        for f in schema.get("fields", []):
            if f.get("enabled", True) and f["name"] not in STANDARD_IMPORT_FIELD_NAMES and f["name"] not in ("location", "qty"):
                extra_field_names.append(f["name"])
    ...
    for row in reader:
        ...
        record = {
            ...
            "mrp": parse_number(norm_row.get('mrp', 0)),
            "cost": parse_number(norm_row.get('cost', 0)),
            "qty": parse_number(norm_row.get('qty', 0)),
...
async def create_session(session: AuditSessionCreate):
    ...
    if client.get("client_type") == "warehouse" and client.get("stock_imported"):
        ...
        if client_stock_records:
            ...
            _expected_cache.invalidate(f"expected_{new_session.id}")
...
async def forward_to_variance(req: ForwardRequest):
    ...
    for location_name, entries in location_groups.items():
        ...
        # Case 3: Existing in variance (any device, including same) → create conflict, pull from variance
        if existing_synced:
        ...
        # Case 4: First time → insert in variance
...
async def get_master_for_session(session_id: str) -> dict:
    ...
    if not client_id:
        return {}
    return await get_master_by_barcode(client_id)
...
async def edit_barcode(data: dict):
    ...
    if report_type in ["barcode-wise", "detailed"]:
        ...
        # Find article by article_code in master
        master_info = None
...
async def search_master_data(client_id: str, q: str = "", field: str = "barcode"):
    ...
    # Try master_products first, fallback to expected_stock
    master = await _load_master_for_client(client_id)
    if not master:
        # Build master from expected_stock
        session_ids = await _get_all_session_ids_for_client(client_id)
        all_exp = []
        for sid in session_ids:
            all_exp.extend(await get_cached_expected(sid))
        for e in all_exp:
            bc = e.get("barcode", "")
...
def _apply_barcode_edits(report, totals, edits, report_type, master_by_barcode):
    ...
    for edit in active:
        ...
        for row in report:
            ...
            if report_type in ["barcode-wise", "detailed"]:
                ...
                match = (ac == edit["original_value"] or ac == "UNMAPPED") and row.get("stock_qty", 0) == 0

            if not match:
                continue

            # Subtract old values from totals
            for k in ["physical_value_mrp", "physical_value_cost", "final_value_mrp", "final_value_cost", "diff_value_mrp", "diff_value_cost"]:
                totals[k] = round(totals.get(k, 0) - row.get(k, 0), 2)

            new_val = edit["new_value"]
            mi = edit.get("master_info", {})

            if report_type in ["barcode-wise", "detailed"]:
                row["barcode"] = new_val
            else:
                row["article_code"] = new_val
            ...
            new_mrp = mi.get("mrp", 0) or 0
            new_cost = mi.get("cost", 0) or 0
            row["mrp"] = new_mrp
            row["cost"] = new_cost

            pq = row.get("physical_qty", 0)
            rq = row.get("reco_qty", 0)
            fq = pq + rq
            row["physical_value_mrp"] = round(pq * new_mrp, 2)
            row["physical_value_cost"] = round(pq * new_cost, 2)
            row["final_value_mrp"] = round(fq * new_mrp, 2)
            row["final_value_cost"] = round(fq * new_cost, 2)
            row["diff_value_mrp"] = round(row["final_value_mrp"] - row.get("stock_value_mrp", 0), 2)
            row["diff_value_cost"] = round(row["final_value_cost"] - row.get("stock_value_cost", 0), 2)
            row["accuracy_pct"] = calc_accuracy(row.get("stock_qty", 0), fq)

            orig_label = "Barcode" if report_type != "article-wise" else "Article"
            row["remark"] = f"{orig_label} Edited (was: {edit['original_value']})"
    ...
    # Mark editable rows (extra items that haven't been edited yet can be edited)
    for row in report:
        if row.get("is_edited"):
            row["is_editable"] = True  # Edited rows can be re-edited
        else:
            row["is_editable"] = (row.get("stock_qty", 0) == 0 and row.get("physical_qty", 0) > 0)
...
def _compute_validated_reco(reco_maps, expected_results, synced_results):
    ...
    Uses the EXACT SAME approach as the Detailed report: iterate all valid item keys and look up reco.
    This ensures ALL reports use identical reco computation — single source of truth.
    """
    all_item_keys = set()
    ...
    for synced in synced_results:
        for s in synced:
            ...
            for item in s.get("items", []):
                all_item_keys.add(f"{loc}|{item['barcode']}")
    
    location_reco = {}
    barcode_reco = {}
    total_reco = 0
    for key in all_item_keys:
        reco = reco_maps["detailed"].get(key, 0)
        if reco != 0:
            loc = key.split("|")[0]
            bc = key.split("|")[1]
            location_reco[loc] = location_reco.get(loc, 0) + reco
            barcode_reco[bc] = barcode_reco.get(bc, 0) + reco
            total_reco += reco
    return {"location": location_reco, "barcode": barcode_reco, "total": total_reco, "item_keys": all_item_keys}
...
async def _load_master_for_client(client_id: str):
    ...
    # Always enrich from expected_stock (has descriptions, categories, etc.)
    session_ids = await _get_all_session_ids_for_client(client_id)
    if session_ids:
        all_expected = []
        for sid in session_ids:
            all_expected.extend(await get_cached_expected(sid))
        for e in all_expected:
            bc = e.get("barcode", "")
            if bc and bc not in master_by_barcode:
                master_by_barcode[bc] = e
    return master_by_barcode

async def _get_extra_columns_for_client(client_id: str):
    """Get all enabled schema columns that aren't fixed report columns (barcode, description, category, mrp, cost).
    This includes standard fields like article_code/article_name AND any custom fields."""
    schema = await db.client_schemas.find_one({"client_id": client_id}, {"_id": 0})
    if not schema:
        return []
    extra = []
    for f in schema.get("fields", []):
        if f.get("enabled", True) and f["name"] not in REPORT_FIXED_COLUMNS:
...
def _merge_custom_fields(row: dict, master: dict, extra_columns: list, expected_cf: dict = None) -> dict:
    """Merge extra schema fields into a report row. Handles both direct fields and custom_fields dict."""
    cf = master.get("custom_fields", {})
    ecf = expected_cf or {}
    for col in extra_columns:
        name = col["name"]
        # First check if it's a direct field on the row (e.g., article_code already set)
        if name in row and row[name]:
            continue
        # Then check master direct fields, then custom_fields, then expected custom_fields
        val = master.get(name, "") or cf.get(name, "") or ecf.get(name, "")
        row[name] = val
    return row
...
@portal_router.get("/reports/consolidated/{client_id}/compare-totals")
async def compare_report_totals(client_id: str):
    """Quick comparison: fetch totals from all 5 reports and compare reco_qty.
    Also runs _compute_validated_reco directly to compare."""
    try:
        session_ids = await _get_all_session_ids_for_client(client_id)
        reco_maps = await _build_reco_maps(client_id)
        
        # Load data same way as reports
        ...
        validated = _compute_validated_reco(reco_maps, expected_results, synced_results)
        
        # Check for empty-location items
        empty_loc_reco = validated["location"].get("", 0)
        none_loc_reco = validated["location"].get("None", 0)
        
        # Check expected data for None vs "" location
        loc_types = {"none_count": 0, "empty_count": 0, "normal_count": 0}
        for expected in expected_results:
            for e in expected:
                loc = e.get("location")
                if loc is None:
                    loc_types["none_count"] += 1
                elif loc == "":
                    loc_types["empty_count"] += 1
                else:
                    loc_types["normal_count"] += 1
        
        results = {}
        for report_func, name in [
            (get_consolidated_bin_wise, "bin_wise"),
            (get_consolidated_detailed, "detailed"),
            (get_consolidated_barcode_wise, "barcode_wise"),
            (get_consolidated_article_wise, "article_wise"),
            (get_consolidated_category_summary, "category_summary"),
        ]:
            try:
                data = await report_func(client_id)
                t = data.get("totals", {})
                results[name] = {"reco_qty": t.get("reco_qty", 0), "physical_qty": t.get("physical_qty", 0), "final_qty": t.get("final_qty", 0)}
            except Exception as e:
                results[name] = {"error": str(e)}
        
        reco_values = [v["reco_qty"] for v in results.values() if "reco_qty" in v]
        
        return {
            "reports": results,
            "all_reco_match": len(set(reco_values)) <= 1,
            "shared_function_result": {
                "total_reco": validated["total"],
                "location_reco_total": sum(validated["location"].values()),
                "barcode_reco_total": sum(validated["barcode"].values()),
                "empty_loc_reco": empty_loc_reco,
                "none_loc_reco": none_loc_reco
            },
            "expected_location_types": loc_types
        }
    except Exception as e:
        return {"error": str(e)}


@portal_router.get("/reports/consolidated/{client_id}/reco-diagnostic")
async def get_reco_diagnostic(client_id: str):
    """Diagnostic endpoint: compare reco computation across all consolidated reports."""
    session_ids = await _get_all_session_ids_for_client(client_id)
    if not session_ids:
        return {"error": "No sessions found for client"}
    
    reco_maps = await _build_reco_maps(client_id)
    
    # Load expected + synced data (same as all reports)
    expected_tasks = [db.expected_stock.find({"session_id": sid}, {"_id": 0}).to_list(100000) for sid in session_ids]
    synced_tasks = [db.synced_locations.find({"session_id": sid}, {"_id": 0}).to_list(100000) for sid in session_ids]
    ...
    for expected in expected_results:
        for e in expected:
            all_item_keys.add(f"{e.get('location', '')}|{e['barcode']}")
    for synced in synced_results:
        for s in synced:
            loc = s["location_name"]
            for item in s.get("items", []):
    ...
    # Compute reco totals by various aggregation methods
    total_reco_from_items = 0
    location_reco = {}
    barcode_reco = {}
    orphaned_reco = []
    
    # Method 1: Iterate all_item_keys and look up reco (same as Detailed report)
    for key in all_item_keys:
        reco = reco_maps["detailed"].get(key, 0)
        if reco != 0:
            total_reco_from_items += reco
            loc = key.split("|")[0]
            bc = key.split("|")[1]
            location_reco[loc] = location_reco.get(loc, 0) + reco
            barcode_reco[bc] = barcode_reco.get(bc, 0) + reco
    
    # Method 2: Check for orphaned reco (entries in reco_maps not in item keys)
    total_orphaned = 0
    for key, reco in reco_maps["detailed"].items():
        if key not in all_item_keys:
            orphaned_reco.append({"key": key, "reco_qty": reco})
            total_orphaned += reco
    
    total_reco_all = sum(reco_maps["detailed"].values())
    
    return {
        "total_reco_entries": len(reco_maps["detailed"]),
        "total_item_keys": len(all_item_keys),
        "total_reco_all_entries": total_reco_all,
        "total_reco_from_valid_items": total_reco_from_items,
        "total_orphaned_reco": total_orphaned,
        "orphaned_entries": orphaned_reco[:20],
        "reco_by_location": {k: v for k, v in sorted(location_reco.items()) if v != 0},
        "location_reco_total": sum(location_reco.values()),
        "barcode_reco_total": sum(barcode_reco.values()),
        "consistency_check": {
            "item_vs_location_match": abs(total_reco_from_items - sum(location_reco.values())) < 0.01,
            "item_vs_barcode_match": abs(total_reco_from_items - sum(barcode_reco.values())) < 0.01,
            "no_orphaned_reco": len(orphaned_reco) == 0
        }
    }
...
async def get_consolidated_bin_wise(client_id: str):
    ...
    validated_reco = _compute_validated_reco(reco_maps, expected_results, synced_results)
    location_reco = validated_reco["location"]
...
async def get_consolidated_detailed(client_id: str):
    ...
    master_task = _load_master_for_client(client_id)
...
async def get_consolidated_barcode_wise(client_id: str):
    ...
    for expected in expected_results:
        for e in expected:
            bc = e["barcode"]
            # When barcode is empty, use article_code as unique key so each item gets its own row
            key = bc if bc else f"_NOBC_{e.get('article_code', '')}"
            if key not in expected_by_barcode:
                expected_by_barcode[key] = {"qty": 0, "description": e.get("description", ""), "category": e.get("category", ""), "mrp": e.get("mrp", 0), "cost": e.get("cost", 0), "article_code": e.get("article_code", ""), "article_name": e.get("article_name", ""), "original_barcode": bc}
                if e.get("custom_fields"):
                    expected_custom_fields[key] = e["custom_fields"]
            expected_by_barcode[key]["qty"] += e.get("qty", 0)
    ...
    # Use shared reco computation — single source of truth for all reports
    validated_reco = _compute_validated_reco(reco_maps, expected_results, synced_results)
    # Mirror reco entries to remapped (post-edit) barcodes so rows aggregated
    # under the NEW value still find their reco (anchored to the ORIGINAL).
    _mirror_reco_to_remapped(validated_reco, loc_remap, global_remap)
    valid_barcode_reco = validated_reco["barcode"]
    ...
    for bc in sorted(all_barcodes):
        ...
        # For _NOBC_ keys, display empty barcode but keep rows separate by article_code
        display_barcode = "" if bc.startswith("_NOBC_") else bc
        master_key = "" if bc.startswith("_NOBC_") else bc
        master = master_by_barcode.get(master_key, {})
        description = master.get("description", "") or exp.get("description", "")
        category = master.get("category", "") or exp.get("category", "")
        ...
        accuracy = calc_accuracy(stock_qty, final_qty)
        in_master = master_key in master_by_barcode
        ...
        row = {
            "barcode": display_barcode, "description": description, "category": category,
            "article_code": article_code, "article_name": article_name,
            "mrp": mrp, "cost": cost,
        ...
        if extra_columns:
            _merge_custom_fields(row, master_by_barcode.get(master_key, {}), extra_columns, expected_custom_fields.get(bc, {}))
        report.append(row)
...
async def get_consolidated_article_wise(client_id: str):
    ...
    # Load all data in parallel (same pattern as other reports)
    expected_tasks = [db.expected_stock.find({"session_id": sid}, {"_id": 0}).to_list(100000) for sid in session_ids]
    synced_tasks = [db.synced_locations.find({"session_id": sid}, {"_id": 0}).to_list(100000) for sid in session_ids]
    all_results = await asyncio.gather(*expected_tasks, *synced_tasks)
    expected_results = all_results[:len(session_ids)]
    ...
    for expected in expected_results:
        for e in expected:
            ...
            if bc not in expected_by_barcode:
                ...
                if e.get("custom_fields"):
                    expected_custom_fields[bc] = e["custom_fields"]
    ...
    validated_reco = _compute_validated_reco(reco_maps, expected_results, synced_results)
    _mirror_reco_to_remapped(validated_reco, loc_remap, global_remap)
    validated_barcode_reco = validated_reco["barcode"]
    ...
    for bc in all_barcodes:
        ...
        if article_code not in article_groups:
            article_groups[article_code] = {"article_name": master.get("article_name", "") or exp.get("article_name", ""), "category": master.get("category", "") or exp.get("category", "Unmapped" if article_code == "UNMAPPED" else ""), "barcodes": [], "stock_qty": 0, "physical_qty": 0, "reco_qty": 0, "mrp": master.get("mrp", 0) or 0, "cost": master.get("cost", 0) or 0}
        ...
        article_groups[article_code]["reco_qty"] += validated_barcode_reco.get(bc, 0)
    ...
    for code in sorted(article_groups.keys()):
        ...
        # For article-wise, pull custom fields from the first barcode's master data or expected stock
        if extra_columns and g["barcodes"]:
            first_bc = sorted(g["barcodes"])[0]
            _merge_custom_fields(row, master_by_barcode.get(first_bc, {}), extra_columns, expected_custom_fields.get(first_bc, {}))
        report.append(row)
...
async def get_consolidated_category_summary(client_id: str):
    ...
    for bc in all_barcodes:
        ...
        physical_qty = physical_by_barcode.get(bc, 0)
        reco_qty = validated_barcode_reco.get(bc, 0)
...
async def get_detailed_report(session_id: str):
    ...
    # Parallel: fetch session, master, expected, synced all at once
    session_task = db.audit_sessions.find_one({"id": session_id}, {"_id": 0})
    expected_task = db.expected_stock.find({"session_id": session_id}, {"_id": 0}).to_list(100000)
    synced_task = db.synced_locations.find({"session_id": session_id}, {"_id": 0}).to_list(100000)
    master_task = get_master_for_session(session_id)
    ...
    for e in expected:
        ...
        if key in expected_map:
            # Accumulate qty for duplicate location|barcode entries (e.g. multiple import batches)
            expected_map[key]["qty"] = expected_map[key].get("qty", 0) + e.get("qty", 0)
        else:
            expected_map[key] = dict(e)
    ...
    # Session-wise: show all items in scanned locations (both expected and physical)
    # This ensures stock totals match between bin-wise summary and detailed report
    all_keys = set(physical_map.keys())
    for key in expected_map:
        loc = key.split("|")[0]
        if loc in scanned_locations:
            all_keys.add(key)
...
async def get_barcode_wise_report(session_id: str):
    ...
    for e in expected:
        ...
        key = bc if bc else f"_NOBC_{e.get('article_code', '')}"
        if key not in expected_by_barcode:
            expected_by_barcode[key] = {"qty": 0, "description": e.get("description", ""), "category": e.get("category", ""), "mrp": e.get("mrp", 0), "cost": e.get("cost", 0), "article_code": e.get("article_code", ""), "article_name": e.get("article_name", "")}
            if e.get("custom_fields"):
                expected_custom_fields[key] = e["custom_fields"]
        expected_by_barcode[key]["qty"] += e.get("qty", 0)
    
    # Session-wise: show barcodes from scanned locations (both expected and physical)
    all_barcodes = set(physical_by_barcode.keys()) | set(expected_by_barcode.keys())
    ...
    for bc in sorted(all_barcodes):
        ...
        display_barcode = "" if bc.startswith("_NOBC_") else bc
        master_key = "" if bc.startswith("_NOBC_") else bc
        master_info = master_by_barcode.get(master_key, {})
        description = exp.get("description") or master_info.get("description") or phy.get("product_name", "")
        category = exp.get("category") or master_info.get("category", "")
        article_code = master_info.get("article_code", "") or exp.get("article_code", "")
        article_name = master_info.get("article_name", "") or exp.get("article_name", "")
        mrp = exp.get("mrp") or master_info.get("mrp", 0)
        cost = exp.get("cost") or master_info.get("cost", 0)
        ...
        accuracy = calc_accuracy(stock_qty, final_qty)
        in_expected = bc in expected_by_barcode
        in_prod_master = master_key in master_by_barcode
        scanned = bc in physical_by_barcode
        remark = generate_remark(stock_qty, final_qty, accuracy, in_master=in_expected, scanned=scanned, in_product_master=in_prod_master, in_expected_stock=in_expected)
...
async def get_article_wise_report(session_id: str):
    ...
    # First pass: build barcode_to_article mapping from ALL expected (for lookup)
    for e in expected:
        bc = e["barcode"]
    ...
    # Also add article info for unmapped scanned barcodes found in master
    for bc in set(barcode_to_article.keys()) & set(b for s in synced for item in s["items"] for b in [item["barcode"]] if b in master_by_barcode and barcode_to_article.get(b) not in expected_by_article):
        article_code = barcode_to_article[bc]
        if article_code not in expected_by_article:
            master_info = master_by_barcode.get(bc, {})
    ...
    # Session-wise: show articles from scanned locations (both expected and physical)
    all_article_codes = set(physical_by_article.keys()) | set(expected_by_article.keys())
...
async def get_category_summary(session_id: str):
    ...
    for e in expected:
        ...
        cost = e.get("cost", 0) or master_info.get("cost", 0)
        mrp = e.get("mrp", 0) or master_info.get("mrp", 0)
        barcode_info[bc] = {"category": cat, "cost": cost, "mrp": mrp}
        if cat not in expected_by_category:
            expected_by_category[cat] = {"qty": 0, "value_mrp": 0, "value_cost": 0, "item_count": 0}
        expected_by_category[cat]["qty"] += e.get("qty", 0)
        expected_by_category[cat]["value_mrp"] += e.get("qty", 0) * mrp
        expected_by_category[cat]["value_cost"] += e.get("qty", 0) * cost
    ...
    for bc in all_barcodes:
        info = barcode_info.get(bc, {"category": "Unmapped", "cost": 0, "mrp": 0})
        cat = info["category"]
        cost = info["cost"]
        mrp = info["mrp"]
        ...
        if cat not in physical_by_category:
            physical_by_category[cat] = {"qty": 0, "value_mrp": 0, "value_cost": 0}
        if cat not in reco_by_category:
            reco_by_category[cat] = 0
        physical_by_category[cat]["qty"] += phy
        physical_by_category[cat]["value_mrp"] += phy * mrp
        physical_by_category[cat]["value_cost"] += phy * cost
    ...
    for cat in sorted(all_categories):
        exp = expected_by_category.get(cat, {"qty": 0, "value_mrp": 0, "value_cost": 0, "item_count": 0})
        phy = physical_by_category.get(cat, {"qty": 0, "value_mrp": 0, "value_cost": 0})
        ...
        avg_mrp = phy["value_mrp"] / physical_qty if physical_qty > 0 else (exp["value_mrp"] / stock_qty if stock_qty > 0 else 0)
        avg_cost = phy["value_cost"] / physical_qty if physical_qty > 0 else (exp["value_cost"] / stock_qty if stock_qty > 0 else 0)
        fv_mrp = phy["value_mrp"] + reco_qty * avg_mrp
        fv_cost = phy["value_cost"] + reco_qty * avg_cost
        dv_mrp = fv_mrp - exp["value_mrp"]
        dv_cost = fv_cost - exp["value_cost"]
        ...
        totals["item_count"] += item_count; totals["stock_qty"] += stock_qty; totals["physical_qty"] += physical_qty
        totals["reco_qty"] += reco_qty; totals["final_qty"] += final_qty; totals["diff_qty"] += diff_qty
        totals["stock_value_mrp"] += exp["value_mrp"]; totals["physical_value_mrp"] += phy["value_mrp"]; totals["final_value_mrp"] += fv_mrp; totals["diff_value_mrp"] += dv_mrp
        totals["stock_value_cost"] += exp["value_cost"]; totals["physical_value_cost"] += phy["value_cost"]; totals["final_value_cost"] += fv_cost; totals["diff_value_cost"] += dv_cost
        
        report.append({
            "category": cat, "item_count": item_count,
            "stock_qty": stock_qty, "stock_value_mrp": round(exp["value_mrp"], 2), "stock_value_cost": round(exp["value_cost"], 2),
            "physical_qty": physical_qty, "physical_value_mrp": round(phy["value_mrp"], 2), "physical_value_cost": round(phy["value_cost"], 2),
            "reco_qty": reco_qty, "final_qty": final_qty, "final_value_mrp": round(fv_mrp, 2), "final_value_cost": round(fv_cost, 2),
            "diff_qty": diff_qty, "diff_value_mrp": round(dv_mrp, 2), "diff_value_cost": round(dv_cost, 2), "accuracy_pct": accuracy, "remark": remark
        })
...
async def get_daily_progress(session_id: str):
    ...
    daily_data = {}
    for s in synced:
        date = s.get("sync_date", s.get("synced_at", "Unknown")[:10] if s.get("synced_at") else "Unknown")
        ...
        daily_data[date]["items"] += s.get("total_items", 0)
        daily_data[date]["quantity"] += s.get("total_quantity", 0)
        daily_data[date]["devices"].add(s.get("device_name", "Unknown"))
...
async def get_consolidated_pending_locations(client_id: str):
    ...
    for sid in session_ids:
        expected = await get_cached_expected(sid)
        for rec in expected:
            loc = rec.get("location", "").strip()
            if loc:
                expected_locations.add(loc)
...
async def get_consolidated_empty_bins(client_id: str):
    ...
    session_ids = await _get_all_session_ids_for_client(client_id)
    if not session_ids:
        return {"total_empty_bins": 0, "by_date": [], "all_empty_locations": []}
    ...
    for sid in session_ids:
        ...
        for s in synced:
            empty_bins.append({
                ...
                "empty_remarks": s.get("empty_remarks", ""),
                "synced_at": s.get("synced_at", ""),
                "sync_date": s.get("sync_date", "unknown"),
                "session_id": sid
            })
    
    # Group by date (same format as session-specific endpoint)
    by_date = {}
    for loc in empty_bins:
        d = loc.get("sync_date", "unknown")
        if d not in by_date:
            by_date[d] = []
        by_date[d].append(loc)
    ...
    return {
        ...
        "by_date": dates,
        "all_empty_locations": empty_bins,
        "session_name": "All Sessions (Consolidated)"
    }
...
async def get_audit_summary():
    ...
    for client in clients:
        ...
        # Top 5 mismatched barcodes (highest absolute diff)
        top_mismatches = []
        try:
...
async def get_master_products_for_scanner(client_id: str):
    """Scanner pulls product master (not stock). Returns barcode, name, price from master_products."""
    if not client_id:
...
async def get_location_master_for_scanner(client_id: str):
    """Scanner pulls location master. Returns all valid locations with zone/floor/area info."""
    if not client_id:
        raise HTTPException(status_code=400, detail="client_id required")
    ...
    locations = []
    for r in records:
        loc_code = r.get("location_code", "")
        if not loc_code:
            continue
        locations.append({
            "code": loc_code,
            "name": r.get("location_name", loc_code),
            "zone": r.get("zone", ""),
            "floor": r.get("floor", ""),
            "area": r.get("area", ""),
...
# ==================== EXPORT ROUTERS ====================
# These routers are imported and included in main server.py
# audit_api_router -> /api/audit
# audit_portal_router -> /api/audit/portal  
# audit_sync_router -> /api/audit/sync

# Note: The seed_test_data function can be called from main server.py if needed
# Helper function to run seed data (can be called from main app)
...
# Note: startup/shutdown events should be registered in main server.py
# Example in server.py:
# from audit_routes import seed_test_data
